  ...
  methods: {
   ...
   getUniqueIdentifier: function(nvrDataTable){
    $(document).on("click", ".editNVR", function(){
      ...
      nvrs.methods.onNVREditButton(nvr_id, data);
    });
  },
   onNVREditButton: function(nvr_id, data) {
    ...
    updateNVRdo: function(){
      ...
      this.$http.patch("/nvrs/" + nvrID, {
        ...
        Vue.notify({group: 'notify', title: 'NVR has been updated'});
        this.show_loading = false;
        this.dataTable.ajax.reload();
        this.editClearFrom();
   ...
   deleteNvr: function(){
    $(document).on("click", ".delNVR", function(){
      ...
      app.$http.delete("/nvrs/" + nvrID, {nvrRow: nvrRow}).then(function (response) {
        nvrRow.remove();
        Vue.notify({group: 'notify', title: 'NVR has been deleted.'});
      }).catch(function (error) {
         return false
      });
    });
    ...
    $(document).off("click").on("click", ".rebootNVR", function(){
      ...
      app.$http.get("/nvrs/" + nvrID, {nvrRow: nvrRow}).then(function (response) {
        if (response.body.status != 201) {
          Vue.notify({group: 'notify', title: response.body.message, type: 'error'});
        }else{
          Vue.notify({group: 'notify', title: 'Nvr has been reboot successfully.'});
        }
      }).catch(function (error) {
          Vue.notify({group: 'notify', title: error.body.message, type: 'error'});
        return false
      });
    });
   },
