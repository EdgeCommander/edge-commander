   ...
   addMap: function() {
    ...
    saveModal: function() {
      ...
      this.$http.post('/sites/new', {
        ...
        Vue.notify({group: 'notify', title: 'Site has been added'});
        this.show_loading = false;
        this.dataTable.ajax.reload();
        this.clearForm();
        $(app.$refs.addmodal).modal("hide");
    ...
    getUniqueIdentifier: function(sitesDataTable){
      $(document).on("click", ".editSite", function(){
        ...
        sites.methods.onSiteEditButton(data);
      });
    },
    ...
    onSiteEditButton: function(data){
      ...
      $(PostCodeid).autocomplete({
        ...
        select: function (event, ui) {
          ...
          marker.setPosition(latlng);
          sites.methods.editMapInitialize();
        }
      });
      });
    },
    updateSitedo: function(){
      ...
      this.$http.patch('/sites/update', {
        ...
        Vue.notify({group: 'notify', title: 'Site has been updated'});
        this.show_loading = false;
        this.dataTable.ajax.reload();
        this.editClearFrom();
    ...
    $(document).off("click").on("click", ".delSite", function(){
      ...
      app.$http.delete("/sites/" + siteID, {siteRow: siteRow}).then(function (response) {
        siteRow.remove();
        Vue.notify({group: 'notify', title: 'Site has been deleted'});
      }).catch(function (error) {
         return false
      });
    });
